...
const PT = {
  ...
  // ── Corredor exterior EXACTO (5 anclajes cls-34 del SVG) ──
  // La ruta especial Sur Occidental ↔ General Sur debe pasar LITERALMENTE por
  // estos 5 puntos, en este orden (Puerta 1 → Playa Roja → Av. J.F. Kennedy →
  // P. Amarilla → Extremo derecho), y luego subir por la Calle Cacica Quilago.
  extPuerta1:    { x: 226.996, y: 348.188 }, // Puerta 1
  extPlayaRoja:  { x: 409.190, y: 363.356 }, // P. Playa Roja (Av. John F. Kennedy)
  extKennedy:    { x: 474.215, y: 393.346 }, // Av. John F. Kennedy (punto más bajo)
  extAmarilla:   { x: 537.973, y: 363.356 }, // P. Amarilla
  extDerecho:    { x: 599.981, y: 348.188 }, // Extremo derecho (hacia Cacica Quilago)
...
// ============================================================
// Corredor Sur (Plazoleta → Av. J.F. Kennedy → Calle Cacica Quilago)
// ------------------------------------------------------------
...
// P2, P3} y la General Sur (P4) ahora REGRESA a la Plazoleta (Puerta 1), baja
// por debajo de la Puerta 1 a la Av. John F. Kennedy, la recorre y sube por la
// Calle Cacica Quilago hasta la Puerta 4 LOCAL. El tramo exterior pasa exacto
// por los 5 anclajes del SVG (extPuerta1 → extPlayaRoja → extKennedy →
// extAmarilla → extDerecho). Se genera con makeSouthCorridorRoute para que
// mapa e indicaciones coincidan siempre.
// ============================================================
...
// Plazoleta (Puerta 1), que es el punto de regreso antes de salir al exterior.
...
// Tramo exterior: Plazoleta → (baja por debajo de la Puerta 1) → Av. John F.
// Kennedy → Calle Cacica Quilago → Puerta 4 LOCAL → General Sur Alta. Pasa
// EXACTO por los 5 anclajes del SVG. A la General Sur SIEMPRE se ingresa por
// Alta; a la Baja NUNCA se entra directo (se sube desde Alta).
const P4_ENTER = [
